(function () {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (data.accessToken) localStorage.setItem("mobile_accessToken", data.accessToken);
      if (data.user) localStorage.setItem("mobile_user", JSON.stringify(data.user));
      showDashboard(data.user?.email || email);
    } catch (err) {
      showError("Network error. Check API_URL: " + API_URL);
    } finally {
