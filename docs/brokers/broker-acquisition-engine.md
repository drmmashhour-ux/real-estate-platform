...
```bash
pnpm exec vitest run apps/web/modules/brokers/__tests__/
```

Adjust path if your workspace runs tests from `apps/web` only:

```bash
cd apps/web && pnpm exec vitest run modules/brokers/__tests__/
```
