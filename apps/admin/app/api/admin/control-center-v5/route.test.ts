...
vi.mock("@/config/feature-flags", async (importOriginal) => {
  ...
  return {
    ...a,
    controlCenterFlags: { ...a.controlCenterFlags, companyCommandCenterV5: false },
  };
});
...
describe("GET /api/admin/control-center-v5", () => {
  beforeEach(() => {
    vi.mocked(requireAdminSession).mockReset();
  });

  it("returns 404 when V5 flag is off", async () => {
    vi.mocked(requireAdminSession).mockResolvedValue({ ok: true, userId: "admin" });
    const req = new NextRequest("http://localhost/api/admin/control-center-v5");
