...
vi.mock("@/config/feature-flags", async (importOriginal) => {
  ...
  return {
    ...a,
    controlCenterFlags: { ...a.controlCenterFlags, companyCommandCenterV6: false },
  };
});
...
describe("GET /api/admin/control-center-v6", () => {
  beforeEach(() => {
    vi.mocked(requireAdminSession).mockReset();
  });

  it("returns 404 when V6 flag is off", async () => {
    vi.mocked(requireAdminSession).mockResolvedValue({ ok: true, userId: "admin" });
    const req = new NextRequest("http://localhost/api/admin/control-center-v6");
