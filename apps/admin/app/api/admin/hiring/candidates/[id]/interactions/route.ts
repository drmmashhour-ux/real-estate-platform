...
import { logInteraction } from "@/src/modules/hiring/pipeline";
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  if (!b.type) return Response.json({ error: "type required" }, { status: 400 });

  try {
    const row = await logInteraction(id, b.type, b.summary ?? "");
