...
const { hi, bud, pr, bl, rk } = vi.hoisted(() => {
  return {
    hi: { disclaimer: "H", dataSources: [""] as string[], roles: [
      { kind: "operations" as const, priority: "low" as const, headcountHint: { min: 0, max: 0, basis: "x" }, rationale: [""], dataTrace: "t" },
    ] },
    bud: { disclaimer: "B", dataSources: [""] as string[], lines: [
      { label: "L", scope: "market" as const, scopeKey: "Q", action: "maintain" as const, priority: "low" as const, rationale: [""], dataTrace: "" },
    ] },
    pr: { disclaimer: "P", dataSources: [""] as string[], prioritize: [{ key: "k", title: "Funnel", priority: "low" as const, rationale: [""], dataTrace: "" }], deprioritize: [] as { key: string; title: string; priority: "low" | "medium" | "high"; rationale: string[]; dataTrace: string; }[] },
    bl: [{ id: "1", kind: "other" as const, severity: "info" as const, title: "ok", rationale: "r", dataTrace: "t", suggestedResponse: "s" }],
    rk: [{ type: "x", severity: "low" as const, message: "m", rationale: "r", mitigation: "m" }],
  };
});
...
describe("buildCorporateStrategySnapshot", () => {
  ...
  it("states advisory in disclaimer", async () => {
