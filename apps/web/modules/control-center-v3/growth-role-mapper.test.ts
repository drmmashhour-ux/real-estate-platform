...
function minimalV1(): AiControlCenterPayload {
  ...
  return {
    systems: {
      ...
      ads: { ...u, shadowMode: false, v8Rollout: false, influenceEnabled: false, primaryEnabled: false, comparisonRuns: null, avgOverlapRate: null, avgDivergenceRate: null, pctRunsRisky: null, anomalyNote: "Anom", topRecommendation: null },
      cro: { ...u, analysisEnabled: false, healthScore: null, topBottleneck: "BN", dropoffSummary: null, recommendationCount: null, readinessNote: null, warningSummary: null },
      ranking: { ...u, totalScore: null, maxScore: null, recommendation: "Rank rec", readinessGatesOk: null, readinessGatesTotal: null, rollbackAny: false, top5Overlap: null, avgRankShift: null, warningsCount: null },
...
describe("mapGrowthRole", () => {
  it("includes CRO bottleneck in blockers when present", async () => {
    vi.mocked(loadAiControlCenterPayload).mockResolvedValue(minimalV1());
    const v2 = await loadCompanyCommandCenterV2Payload({});
    const g = mapGrowthRole(v2);
    expect(g.topBlockers.some((b) => b.label.includes("BN"))).toBe(true);
    expect(g.role).toBe("growth");
  });
});
