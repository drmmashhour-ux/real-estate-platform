...
describe("mapFounderRole", () => {
  it("surfaces executive opportunities as priorities", async () => {
    vi.mocked(loadAiControlCenterPayload).mockResolvedValue(minimalV1());
    const v2 = await loadCompanyCommandCenterV2Payload({});
    const f = mapFounderRole(v2);
    expect(f.topPriorities.some((p) => p.label === "O1")).toBe(true);
