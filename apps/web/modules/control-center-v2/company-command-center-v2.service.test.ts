...
import { loadCompanyCommandCenterV2Payload } from "./company-command-center-v2.service";
...
describe("loadCompanyCommandCenterV2Payload", () => {
  it("wraps V1 payload with tab slices", async () => {
    vi.mocked(loadAiControlCenterPayload).mockResolvedValue(minimalV1());
    const p = await loadCompanyCommandCenterV2Payload({});
    expect(p.v1).toBeDefined();
    expect(p.executive.quickKpis.length).toBeGreaterThan(0);
    expect(p.rollouts.rows.length).toBeGreaterThan(0);
    expect(p.meta.sourcesUsed).toContain("control_center_v1:embedded");
  });
});
