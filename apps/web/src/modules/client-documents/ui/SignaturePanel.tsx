  ...
  return (
    <div className="rounded-xl border border-white/10 bg-black/20 p-3">
      ...
      <div className="mt-2 space-y-2">
        ...
        <button type="button" disabled={!confirmed || !name.trim() || !email.trim()} onClick={() => onSign(name.trim(), email.trim())} className="w-full rounded-md bg-premium-gold px-3 py-2 text-xs font-medium text-black disabled:opacity-50">Submit signature</button>
      </div>
    </div>
  );
}
