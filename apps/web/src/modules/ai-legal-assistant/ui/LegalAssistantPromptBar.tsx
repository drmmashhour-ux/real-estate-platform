...
export function LegalAssistantPromptBar({ onSend }: { onSend: (message: string) => void }) {
  ...
  return (
    <div className="flex gap-2">
      ...
      <button type="button" onClick={() => { if (!text.trim()) return; onSend(text.trim()); setText(""); }} className="rounded-lg bg-[#C9A646] px-3 py-2 text-xs font-medium text-black">Ask</button>
    </div>
  );
}
