    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
      <div className="space-y-3 rounded-xl border border-white/10 bg-black/20 p-4">
        ...
        {template?.fields.map((field) => (
          <div key={field.key} className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              <button type="button" className="text-xs text-premium-gold" onClick={() => suggest(field.key)}>AI suggest</button>
            </div>
            <input
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={generate} className="rounded-lg bg-premium-gold px-3 py-2 text-xs text-black">Generate</button>
        </div>
      </div>
