...
export function BestPropertyCard({ selection }: { selection: PropertySelectionResult }) {
  return (
    <div className="rounded-xl border border-white/10 bg-black/30 p-4">
      <p className="text-xs uppercase tracking-wide text-[#C9A646]">{selection.category.replace(/_/g, " ")}</p>
      ...
      <Link href={`/analysis/${selection.city.toLowerCase().replace(/\s+/g, "-")}/${selection.listingId}`} className="mt-3 inline-flex text-xs text-[#C9A646] hover:underline">
        Open analysis
      </Link>
    </div>
  );
