...
## FSBO (sell-by-yourself) — optional

- `FSBO_PUBLISH_PRICE_CENTS` — default `9900` ($99). One-time Stripe Checkout per draft listing (`paymentType: fsbo_publish`).
- `FSBO_ALLOW_UNPAID_PUBLISH=true` — **dev only**: activate listing without Stripe when secret key is missing (do not use in production).
...
- [ ] `apps/web/public/logo.png` is **committed** (generated from `logo.svg` if missing):
  ```bash
  cd apps/web && node -e "require('sharp')(require('fs').readFileSync('public/logo.svg')).png().resize(160,160).toFile('public/logo.png').then(()=>console.log('ok'))"
...
See also `apps/web/.env.production.example`.
