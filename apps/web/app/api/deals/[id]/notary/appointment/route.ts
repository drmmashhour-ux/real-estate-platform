...
import { setNotaryAppointment } from "@/modules/notary-coordination/closing-appointment.service";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  let body: { appointmentAt?: string | null };
  try {
  ...
  const at = body.appointmentAt ? new Date(body.appointmentAt) : null;
  const row = await setNotaryAppointment(dealId, at, userId);
