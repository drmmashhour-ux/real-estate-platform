...
import { approveSuggestion } from "@/modules/negotiation-copilot/negotiation.service";
...
export async function POST(_request: Request, context: { params: Promise<{ id: string; scenarioId: string }> }) {
  ...
  const updated = await approveSuggestion({ dealId, suggestionId: scenarioId, actorUserId: auth.userId });
  await logBrokerWorkspaceEvent({
    actorUserId: auth.userId,
    actionKey: brokerWorkspaceAuditKeys.negotiationScenarioApproved,
    dealId,
    payload: { scenarioId },
  });
