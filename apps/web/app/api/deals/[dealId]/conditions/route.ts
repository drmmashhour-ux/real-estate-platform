...
import { createCondition, listConditions } from "@/modules/deals/deal-conditions.service";
...
export async function GET(_req: Request, context: { params: Promise<{ dealId: string }> }) {
  ...
  try {
    ...
    logError("[api.deals.conditions.get]", { error: e });
...
export async function POST(req: Request, context: { params: Promise<{ dealId: string }> }) {
  ...
  try {
    ...
    logError("[api.deals.conditions.post]", { error: e });
