...
/**
 * Upload ID document for a party row in seller declaration (stored URL in JSON).
 * Files go to Supabase (or local) under `party-id/{partyId}/`.
 */
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!partyId || partyId.length > 120) {
    return Response.json({ error: "partyId required" }, { status: 400 });
  }
  ...
  try {
    const { url, fileName } = await uploadFsboListingDoc({
      ...
      folder: `party-id/${partyId.replace(/[^a-zA-Z0-9_-]/g, "")}`,
    });
    return Response.json({
      ok: true,
      url,
      fileName,
      idDocumentVerificationStatus: "pending" as const,
    });
  } catch (e) {
