...
import { markViewed } from "@/modules/transactions/transaction-signature.service";
...
export async function POST(req: Request, context: { params: Promise<{ signerId: string }> }) {
  ...
  try {
    ...
    const updated = await markViewed(signerId, ip, ua);
    return NextResponse.json({ signer: updated });
  } catch (e) {
    logError("[signatures.signer.view]", { error: e });
