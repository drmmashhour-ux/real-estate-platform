...
import { getNegotiationChainForListing, rejectVersion } from "@/src/modules/negotiation-chain-engine/application/negotiationChainService";
...
export async function POST(request: Request, context: { params: Promise<{ chainId: string; versionId: string }> }) {
  ...
  try {
    const version = await rejectVersion(chainId, versionId);
