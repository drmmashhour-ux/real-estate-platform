...
vi.mock("@/config/feature-flags", async (importOriginal) => {
  ...
  return {
    ...a,
    controlCenterFlags: { ...a.controlCenterFlags, aiControlCenterV1: false },
  };
});
...
describe("GET /api/admin/control-center", () => {
  ...
  it("returns 404 when AI control center flag is off", async () => {
    vi.mocked(requireAdminSession).mockResolvedValue({ ok: true, userId: "admin" });
    const req = new NextRequest("http://localhost/api/admin/control-center");
