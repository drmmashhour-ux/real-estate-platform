...
import { rangeFromDays } from "@/modules/metrics/metrics.service";
...
export async function GET(request: NextRequest) {
  ...
  const gate = assertCommandCenterEnabled("mi");
  ...
  const to = sp.get("to");
  const segment = parseSegmentFromSearchParams(sp);
  const req =
    from && to
      ? { from: new Date(from), toExclusive: new Date(to), segment }
      : { ...rangeFromDays(days), segment };
  const mi = await getMarketIntelligenceBundle(req);
  return NextResponse.json(mi);
}
