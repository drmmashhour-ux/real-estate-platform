...
import { loadCompanyCommandCenterV5Payload } from "@/modules/control-center-v5/company-command-center-v5.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const payload = await loadCompanyCommandCenterV5Payload({
      days,
      limit,
      offsetDays,
      previousOffsetDays,
    ...
    return NextResponse.json(
      { error: "Control center V5 failed", detail: e instanceof Error ? e.message : String(e) },
      { status: 500 },
    );
  }
}
