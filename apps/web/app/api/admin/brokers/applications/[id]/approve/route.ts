...
/**
 * POST /api/admin/brokers/applications/:id/approve – Approve broker application.
 * Sets user.role = BROKER, user.brokerStatus = VERIFIED.
 */
export async function POST(
  ...
  try {
    ...
    await prisma.$transaction([
      ...
      prisma.user.update({
        where: { id: app.userId },
        data: { role: "BROKER", brokerStatus: "VERIFIED", accountStatus: "ACTIVE" },
      }),
    ]);

    return Response.json({ success: true, message: "Broker approved." });
  } catch (e) {
    console.error(e);
