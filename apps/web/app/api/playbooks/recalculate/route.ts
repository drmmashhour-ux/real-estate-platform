...
export async function POST(req: NextRequest) {
  ...
  if (playbookId) {
    ...
    if (!result.ok) {
      ...
      console.error("[playbook]", "recalculate_single_failed", { playbookId, error: result.error });
      return NextResponse.json({ ok: false, error: result.error });
    }
    // eslint-disable-next-line no-console
    console.log("[playbook]", "recalculate_api_single", { playbookId, empty: result.empty });
    return NextResponse.json({ ok: true, scope: "single" as const, result });
  }

  const result = await recalculateAllPlaybookStats();
  // eslint-disable-next-line no-console
  console.log("[playbook]", "recalculate_api_all", { processed: result.processed, failed: result.failed });
  return NextResponse.json({ ok: true, scope: "all" as const, result });
}
