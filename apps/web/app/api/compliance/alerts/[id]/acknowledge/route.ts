...
/** Mark a `ComplianceRiskEvent` as reviewed (alert acknowledgment). */
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const row = await prisma.complianceRiskEvent.findUnique({ where: { id: id.trim() } });
  if (!row) {
  ...
  const updated = await prisma.complianceRiskEvent.update({
    where: { id: row.id },
    data: {
      reviewed: true,
