...
import { isDealAnalyzerEnabled, isDealAnalyzerScenariosEnabled } from "@/modules/deal-analyzer/config";
import { getDealAnalysisPublicDto } from "@/modules/deal-analyzer/application/getDealAnalysis";

export const dynamic = "force-dynamic";

export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  if (!isDealAnalyzerEnabled() || !isDealAnalyzerScenariosEnabled()) {
    return NextResponse.json({ error: "Deal Analyzer scenarios disabled" }, { status: 503 });
  }
  ...
  return NextResponse.json({
    scenarios: analysis?.phase2?.scenarios ?? [],
  });
}
