...
import { runLeadBodySchema } from "@/modules/autonomous-marketplace/api/zod-schemas";
...
export async function POST(req: Request) {
  ...
  const parsed = runLeadBodySchema.safeParse(json);
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid body", issues: parsed.error.flatten() }, { status: 400 });
  }

  try {
    const run = await autonomousMarketplaceEngine.runForLead(parsed.data.leadId, {
