...
/**
 * POST — rebuild BNHub `BnhubRevenueMetricSnapshot` rows for a host (last 30 UTC days rolled into KPI row).
 * Auth: signed-in host (implicit user) **or** `Authorization: Bearer CRON_SECRET` + JSON `{ userId }` for nightly jobs.
 */
