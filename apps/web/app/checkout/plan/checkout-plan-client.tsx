...
export function CheckoutPlanClient() {
  ...
  if (!valid || !hub || !plan) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        ...
        <Link
          href="/pricing"
          className="mt-8 inline-flex rounded-xl bg-gradient-to-r from-[#C9A646] to-[#a88a2e] px-6 py-3 text-sm font-semibold text-black hover:brightness-110"
        >
          View pricing
        </Link>
      </div>
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-16 sm:py-20">
      <p className="text-center text-xs font-semibold uppercase tracking-wider text-[#C9A646]/90">Checkout</p>
      ...
      <div className="mt-10 rounded-2xl border border-[#C9A646]/30 bg-gradient-to-b from-[#C9A646]/10 to-black/40 p-6 text-left">
        <dl className="space-y-3 text-sm">
          ...
          <div className="flex justify-between gap-4">
            ...
            <dd className="font-medium text-[#C9A646]">{planName}</dd>
          </div>
          <div className="flex justify-between gap-4">
      ...
      <div className="mt-8 flex flex-col gap-3">
        <Link
          href={registerHref}
          className="inline-flex w-full items-center justify-center rounded-xl bg-gradient-to-r from-[#C9A646] via-[#d4b45c] to-[#a88a2e] px-5 py-3.5 text-center text-sm font-semibold text-black shadow-lg shadow-[#C9A646]/20 hover:brightness-110"
        >
          Create account &amp; continue
        </Link>
        <Link
          href={loginHref}
          className="inline-flex w-full items-center justify-center rounded-xl border border-white/20 px-5 py-3.5 text-sm font-semibold text-white hover:border-[#C9A646]/40 hover:bg-white/5"
        >
          Sign in
        </Link>
        <Link href="/pricing" className="pt-2 text-center text-sm text-slate-500 hover:text-[#C9A646]">
          ← Back to pricing
        </Link>
      </div>
    </div>
