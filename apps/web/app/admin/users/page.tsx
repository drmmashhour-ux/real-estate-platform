...
import { getAdminUsers } from "@/lib/admin/control-center";
...
export default async function AdminUsersControlPage({ searchParams }: { searchParams: Promise<Sp> }) {
  ...
  const rows = await getAdminUsers({ search, role, accountStatus, verified });
    ...
    <LecipmControlShell>
      <div className="space-y-8">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">Accounts, roles, and verification snapshot.</p>
        </div>
