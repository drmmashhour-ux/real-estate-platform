...
export default async function AiControlCenterPage() {
  ...
  const [models, alerts, decisions, fraudCounts, versionMetrics, pricingStats, aiWriterWeek, aiWriterAll] =
    await Promise.all([
      listAiModels(),
      ...
      getModelVersionMetrics(),
      getPricingRecommendationStats(),
      prisma.aiWriterUsageLog.count({ where: { createdAt: { gte: sinceWeek } } }),
      prisma.aiWriterUsageLog.count(),
    ]);
      ...
      <section className="border-b border-slate-800 bg-slate-950/80">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
          ...
          <div className="mt-4 flex flex-wrap gap-4">
            <Link href="/admin/ai-control-center" className="text-sm font-medium text-amber-400 hover:text-amber-300">
              AI Control Center Dashboard
            </Link>
            <Link href="/admin/ai-operations" className="text-sm font-medium text-amber-400 hover:text-amber-300">
              AI Operations (Queue)
            </Link>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
          ...
          <div className="mt-4 flex flex-wrap gap-4">
            ...
            <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-2">
              <span className="text-slate-400">AI Writing Assistant (7d / all):</span>{" "}
              <span className="font-semibold text-amber-200">
                {aiWriterWeek} / {aiWriterAll}
              </span>
              <p className="mt-1 text-[10px] text-slate-500">POST /api/ai/write — listings, contact, mortgage notes</p>
            </div>
          </div>
        </div>
      </section>
