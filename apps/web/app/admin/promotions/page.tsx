...
export default async function AdminPromotionsPage() {
  const campaigns = await getPromotionCampaigns({ limit: 50 });
  ...
  return (
    <main className="bg-slate-950 text-slate-50">
      <section className="border-b border-slate-800 bg-slate-950/80">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
          ...
          <p className="mt-3 text-sm text-slate-400">
            Featured, sponsored, and boost campaigns. Use POST /api/admin/promotions and /promotions/promote to create.
          </p>
          <div className="mt-4">
      ...
      <section className="bg-slate-950/80">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
          {campaigns.length === 0 ? (
            <p className="text-sm text-slate-500">No campaigns. Create via API.</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-700 bg-slate-900/80">
                    ...
                    <th className="px-4 py-3 text-left font-medium text-slate-400">Promotions</th>
                ...
                <tbody>
                  {campaigns.map((c) => (
                    <tr key={c.id} className="border-b border-slate-800/80">
                      ...
                      <td className="px-4 py-3 text-slate-300">{c._count.promotions}</td>
