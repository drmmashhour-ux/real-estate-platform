...
function comparableFromListing(l: Listing) {
  return {
    title: l.title,
    ...
    city: l.city,
    country: l.country,
    nightPrice: (l.nightPriceCents / 100).toString(),
    beds: l.beds.toString(),
    baths: l.baths.toString(),
    maxGuests: l.maxGuests.toString(),
    photos: l.photos.join(", "),
    conditionOfProperty: l.conditionOfProperty ?? "",
...
export function EditListingForm({ listing }: { listing: Listing }) {
  ...
  const isDirty = useMemo(
    () => JSON.stringify(currentComparable) !== JSON.stringify(comparableFromListing(listing)),
    [listing, currentComparable]
  );

  function discardChanges() {
    const b = comparableFromListing(listing);
    setTitle(b.title);
    setDescription(b.description);
    setAddress(b.address);
    setCity(b.city);
    setCountry(b.country);
    setNightPrice(b.nightPrice);
    setBeds(b.beds);
    setBaths(b.baths);
    setMaxGuests(b.maxGuests);
    setPhotos(b.photos);
    setConditionOfProperty(b.conditionOfProperty);
    setKnownIssues(b.knownIssues);
    setError("");
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
      {isDirty ? (
        <div className="flex flex-col gap-3 rounded-xl border border-amber-500/30 bg-amber-950/25 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-amber-100">
            You have unsaved edits. Save to update BNHUB, or discard to restore the last saved details.
          </p>
          <button
            type="button"
            onClick={discardChanges}
            className="shrink-0 rounded-lg border border-amber-500/40 px-3 py-1.5 text-sm font-medium text-amber-200 hover:bg-amber-950/50"
          >
            Discard changes
          </button>
        </div>
