...
export function BNHubCheckoutClient({
  ...
  bookingReassuranceExperiment?: { experimentId: string; variantId: string };
  reassuranceHint?: string | null;
}) {
  ...
  useEffect(() => {
    if (journeyStarted.current) return;
    journeyStarted.current = true;
    void fetch("/api/journey/event", {
      method: "POST",
      credentials: "same-origin",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "booking_started", listingId }),
    }).catch(() => {});
    if (bookingReassuranceExperiment) {
      void fetch("/api/experiments/track", {
        method: "POST",
        credentials: "same-origin",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          experimentId: bookingReassuranceExperiment.experimentId,
          variantId: bookingReassuranceExperiment.variantId,
          eventName: "booking_start",
          metadata: { listingId, surface: "bnhub_checkout" },
        }),
      }).catch(() => {});
    }
  }, [listingId, bookingReassuranceExperiment]);
  ...
  async function submitBooking() {
    ...
    try {
      ...
      track(TrackingEvent.BOOKING_CLICK, { meta: { listingId, source: "bnhub_checkout" } });
  ...
  return (
    <form onSubmit={handleSubmit} className="animate-fade-in space-y-6">
      ...
      <BnhubPriceMayIncreaseHint variant="dark" />
      ...
      {breakdown && (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 shadow-lg shadow-black/20">
          <h2 className="text-lg font-semibold text-slate-200">Your stay cost</h2>
          ...
          <div className="mt-3 space-y-2 text-sm">
            <div className="flex justify-between text-slate-300">
              <span>Price per night (lodging avg.)</span>
              <span>
                $
                {(
                  (breakdown.lodgingSubtotalAfterDiscountCents ?? breakdown.subtotalCents) /
                  Math.max(1, breakdown.nights) /
                  100
                ).toFixed(0)}
              </span>
            </div>
            ...
            <p className="mt-2 text-xs text-slate-500">
              Includes lodging, cleaning, taxes, and platform service fee as shown — no surprise charges on this step.
            </p>
          </div>
        </div>
      )}

      {breakdown && hostPayoutReady ? (
        <BnhubTrustSignals
      ...
      <BnhubStripeTrustHint className="w-full" variant="prominent" tone="dark" />

      {guestId ? (
        <p className="text-sm text-slate-400">
          {reassuranceHint?.trim() ||
            "👉 You won\u2019t be charged yet — continue to review the total on the secure payment page."}
        </p>
      ) : null}
      ...
      <div className="flex flex-wrap gap-3">
        {!guestId ? (
          ...
          >
            {loading ? "Securing your booking…" : "Secure your booking"}
          </button>
        )}
        <Link
