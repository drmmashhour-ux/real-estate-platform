...
export function ListingImageGallery({
  ...
  /** Platform-verified BNHUB listing — show trust badge on the hero photo. */
  verified?: boolean;
  ...
  if (!photos.length) {
    return (
      <div className="flex aspect-[5/3] min-h-[220px] w-full flex-col items-center justify-center gap-2 rounded-2xl border border-neutral-700 bg-slate-900/80 px-4 text-center text-slate-400">
        <span className="text-sm font-medium text-slate-300">Photos aren&apos;t available yet</span>
        <span className="max-w-xs text-xs leading-relaxed text-slate-500">
          The host may still be uploading images. You can still review details and request to book.
        </span>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60">
        <img
          ...
          className="aspect-[5/3] max-h-[min(78vh,560px)] w-full object-cover sm:aspect-[16/9] lg:max-h-[min(72vh,620px)]"
        />
        ...
        {verified ? <VerifiedListingBadge variant="light" layout="overlay-right" /> : null}
        {photos.length > 1 && (
          <>
            <button
              type="button"
