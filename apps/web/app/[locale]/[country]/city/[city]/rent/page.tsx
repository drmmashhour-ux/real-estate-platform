...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!slug) return { title: "Rent" };
  return buildCityIntentMetadata("rent", slug, { canonicalPath: `/city/${slug}/rent` });
}

export default async function CityRentPage({ params }: PageProps) {
  const { city } = await params;
  return <CityIntentLanding intent="rent" cityParam={city} pathVariant="city" />;
}
