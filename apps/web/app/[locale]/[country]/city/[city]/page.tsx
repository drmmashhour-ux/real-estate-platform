...
export default async function CityPage({ params }: PageProps) {
  ...
  const mapListings: MapListing[] = bnhubResult.listings
    ...
    .map((l) => ({
      ...
      address: [l.city, l.region].filter(Boolean).join(", ") || null,
      ...
      <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        ...
        <section aria-labelledby="bnhub-city-heading">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              <h2
                id="bnhub-city-heading"
                className="flex flex-wrap items-center gap-3 text-2xl font-semibold text-slate-900"
              >
                <BnHubLogoMark size="sm" className="max-h-10 max-w-[160px]" />
