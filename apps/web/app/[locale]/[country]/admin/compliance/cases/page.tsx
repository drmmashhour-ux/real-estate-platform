...
export default async function AdminComplianceCasesPage() {
  const userId = await requireAdminControlUserId();
  if (!complianceAdminFlags.adminComplianceCommandCenterV1) {
    return (
      <HubLayout title="Compliance cases" hubKey="admin" navigation={hubNavigation.admin}>
        <p className="text-zinc-400">Compliance command center is disabled.</p>
      </HubLayout>
    );
  }
  ...
  return (
    <HubLayout title="Compliance cases" hubKey="admin" navigation={hubNavigation.admin}>
      <div className="mx-auto max-w-7xl px-2 py-2 sm:px-4">
        <ComplianceCommandCenter data={data} navActive="/admin/compliance/cases" mode="cases" />
      </div>
    </HubLayout>
  );
}
