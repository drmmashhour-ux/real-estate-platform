...
export default async function Page() {
  ...
  const plans = await prisma.bnhubMembershipPlan.findMany({ orderBy: { name: "asc" } });
  return (
    <HubLayout title="BNHUB memberships" hubKey="admin" navigation={hubNavigation.admin}>
        ...
        <ul className="divide-y divide-zinc-800 rounded-xl border border-zinc-800">
          {plans.map((p) => (
            <li key={p.id} className="p-4 text-sm">
              ...
              <span className="text-zinc-500">{p.membershipCode}</span>
            </li>
          ))}
        </ul>
      </div>
