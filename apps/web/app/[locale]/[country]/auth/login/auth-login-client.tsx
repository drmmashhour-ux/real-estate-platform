...
function AuthLoginInner() {
  ...
  return (
    ...
    >
      {registered ? (
        <div className="rounded-lg border border-emerald-500/35 bg-emerald-950/35 p-3 text-sm leading-relaxed text-emerald-100">
          <p className="font-medium text-emerald-200">Account created</p>
          <p className="mt-1">
            <strong className="text-emerald-100">Next:</strong> open the confirmation email and tap the link. Then sign
            in here. After sign-in you’ll answer one quick question — or open{" "}
            <a href="/dashboard/guest-hub" className="font-medium text-emerald-300 underline">
              Guest hub
            </a>{" "}
            for saved stays and trips.
          </p>
        </div>
      ) : null}
      {verified ? (
      ...
      {err ? (
        <p
          role="alert"
          className="rounded-xl border border-red-500/35 bg-red-950/40 px-3 py-2.5 text-sm text-red-200"
        >
          {err}
        </p>
      ) : null}
...
function LoginFormSkeleton() {
  return (
    <div className="mt-8 space-y-4 rounded-2xl border border-white/10 bg-[#121212] p-6" aria-hidden>
      <div className="h-4 w-24 animate-pulse rounded bg-white/10" />
      <div className="h-10 w-full animate-pulse rounded-xl bg-white/10" />
      <div className="h-4 w-20 animate-pulse rounded bg-white/10" />
      <div className="h-10 w-full animate-pulse rounded-xl bg-white/10" />
      <div className="h-11 w-full animate-pulse rounded-xl bg-premium-gold/25" />
      <p className="text-center text-xs text-slate-500">Loading sign-in…</p>
    </div>
  );
}
