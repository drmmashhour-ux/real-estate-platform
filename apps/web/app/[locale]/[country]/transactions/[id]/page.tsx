    ...
    <main className="min-h-screen bg-slate-950 text-slate-50">
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <TransactionDetailClient
          ...
          inspectionIssues={
            (() => {
              const last = [...tx.events].reverse().find((e) => e.eventType === "inspection_issues_marked");
              const data = last?.eventData as { issues?: string[] } | null;
              return data?.issues ?? [];
            })()
          }
          hasInspectionDoc={tx.documents.some((d) => d.documentType === "inspection_conditions")}
        />
      </div>
    </main>
  );
