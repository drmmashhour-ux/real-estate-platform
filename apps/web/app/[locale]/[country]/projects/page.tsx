...
/** Anchor for smooth scroll when the client switches to Map view (below long filter panels). */
const PROJECTS_MAP_SECTION_ID = "projects-map-section";
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    const switchedToMap = prevViewModeRef.current !== "carte" && viewMode === "carte";
    prevViewModeRef.current = viewMode;
    if (!switchedToMap) return;
    let cancelled = false;
    const tryScroll = (attempt: number) => {
      if (cancelled) return;
      const el = document.getElementById(PROJECTS_MAP_SECTION_ID);
      if (el) {
        el.scrollIntoView({ behavior: "smooth", block: "start" });
        return;
      }
      if (attempt < 14) window.setTimeout(() => tryScroll(attempt + 1), 120);
    };
    const start = window.setTimeout(() => tryScroll(0), 280);
    return () => {
      cancelled = true;
      clearTimeout(start);
    };
    ...
    <main className="min-h-screen bg-[#0a0e17] text-slate-50">
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <div
            id={PROJECTS_MAP_SECTION_ID}
            className="w-full scroll-mt-28 lg:scroll-mt-24"
          >
            {HAS_GOOGLE_MAPS_KEY ? (
              <ProjectsGoogleMap
