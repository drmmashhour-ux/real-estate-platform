  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/mortgage/expert/profile", {
        ...
        body: JSON.stringify({ name, phone, company, bio, title, licenseNumber }),
      });
      const j = (await res.json().catch(() => ({}))) as Expert & { error?: string };
      if (!res.ok) {
        setMsg(typeof j.error === "string" ? j.error : "Save failed");
        return;
      }
      setExpert(j as Expert);
        ...
        <div className="min-w-0 flex-1 space-y-4">
          <p className="text-xs text-[#737373]">
            ...
            <span className="text-slate-300">
              {expert.expertVerificationStatus === "verified"
                ? "Verified"
                : expert.expertVerificationStatus === "pending_review"
                  ? "In review"
                  : "Not complete"}
            </span>
          </p>
          {expert.expertVerificationStatus !== "verified" ? (
            <p className="text-xs text-sky-400/90">
              <a href="/dashboard/expert/verification" className="font-semibold underline">
                Complete verification
              </a>{" "}
              to appear publicly and receive routed leads.
            </p>
          ) : null}
          <div>
          ...
          <div>
            <label className="text-xs font-semibold uppercase text-premium-gold/90">AMF licence #</label>
            <input
              value={licenseNumber}
              onChange={(e) => setLicenseNumber(e.target.value)}
              disabled={expert.expertVerificationStatus === "pending_review"}
              className="mt-1 w-full rounded-xl border border-white/15 bg-[#0B0B0B] px-3 py-2 text-sm disabled:opacity-50"
            />
            {expert.expertVerificationStatus === "pending_review" ? (
              <p className="mt-1 text-[11px] text-amber-200/80">Locked while under review.</p>
            ) : null}
          </div>
          <div>
