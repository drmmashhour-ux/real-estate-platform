...
export const metadata = {
  title: "AI CEO · Decisions",
};

export default async function CeoDecisionsPage() {
  const userId = await getGuestId();
  if (!userId) redirect("/login?next=/dashboard/ceo/decisions");
  ...
  return (
    <main className="min-h-screen bg-[#050508] px-4 py-10 text-slate-100">
      <div className="mx-auto max-w-6xl">
        ...
        <h1 className="mt-2 text-3xl font-bold text-white">Decision queue</h1>
        <p className="mt-1 text-sm text-slate-400">Approve or reject material actions before execution.</p>
        <div className="mt-8">
          <CeoSubnav />
          <CeoDecisionsClient />
        </div>
