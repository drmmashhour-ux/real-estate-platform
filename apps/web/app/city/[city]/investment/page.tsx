...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!slug) return { title: "Investment" };
  return buildCityIntentMetadata("investment", slug, { canonicalPath: `/city/${slug}/investment` });
}

export default async function CityInvestmentPage({ params }: PageProps) {
  const { city } = await params;
  return <CityIntentLanding intent="investment" cityParam={city} pathVariant="city" />;
}
