...
import { tracedCoreDB, tracedMonolithDB } from "./traced-clients";
...
function ensureDbCircuitOrThrow(): void {
  if (process.env.NEXT_PUBLIC_DISABLE_DB === "true") return;
  if (isDbCircuitOpen()) {
...
export async function queryWithRetry(
  ...
  if (process.env.NEXT_PUBLIC_DISABLE_DB === "true") {
    if (process.env.NODE_ENV !== "production") {
      console.warn("[DB DISABLED] queryWithRetry skipped");
    }
    return { rows: [], rowCount: 0 } as QueryResult<Record<string, unknown>>;
  }
