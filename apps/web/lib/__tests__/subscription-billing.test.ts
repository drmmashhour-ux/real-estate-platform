...
vi.mock("@/lib/db", () => ({
  prisma: {
    planSubscription: { findFirst: vi.fn(), findMany: vi.fn() },
    subscriptionPlan: { findUnique: vi.fn(), findMany: vi.fn() },
    planBillingEvent: { create: vi.fn() },
  },
}));
...
describe("Subscription billing", () => {
  beforeEach(() => {
    vi.mocked(prisma.planSubscription.findFirst).mockReset();
  ...
  describe("getActiveSubscription", () => {
    it("returns null when no active subscription", async () => {
      vi.mocked(prisma.planSubscription.findFirst).mockResolvedValue(null);
    ...
    it("returns subscription when active", async () => {
      ...
      vi.mocked(prisma.planSubscription.findFirst).mockResolvedValue({
        id: "S1",
        userId: "user-1",
        planId: "P1",
        plan,
