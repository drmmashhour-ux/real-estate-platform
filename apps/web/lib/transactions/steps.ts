...
import { recordRevenueEvent } from "@/lib/monetization";
...
export async function completeStep(input: CompleteStepInput): Promise<void> {
  ...
  if (pending === 0) {
    const transaction = await prisma.realEstateTransaction.findUnique({
    ...
    if (transaction?.offerPrice && transaction.offerPrice > 0) {
      const feeResult = calculateTransactionFee({ transactionValueCents: transaction.offerPrice });
      if (feeResult.platformFeeCents > 0) {
        void recordRevenueEvent({
          entityType: "transaction",
          entityId: input.transactionId,
          revenueType: "transaction_fee",
          amountCents: feeResult.platformFeeCents,
          currency: "USD",
          status: "realized",
          sourceReference: input.transactionId,
        });
      }
    }
  } else {
