...
export async function recommendPricing(params: {
  ...
  let recommendedNightlyCents: number | undefined;
  let recommendedSalePriceRange: { min: number; max: number } | undefined;
  let recommendedRentRange: { min: number; max: number } | undefined;
  ...
  if (pid) {
    ...
    if (saleVal?.estimatedValue) {
      recommendedSalePriceRange = {
        min: Math.round(saleVal.estimatedValue * 0.95),
        max: Math.round(saleVal.estimatedValue * 1.05),
      };
      factors.push("Sale valuation available");
    }
    if (rentVal?.monthlyRentEstimate) {
      recommendedRentRange = {
        min: Math.round(rentVal.monthlyRentEstimate * 0.95),
        max: Math.round(rentVal.monthlyRentEstimate * 1.05),
      };
      factors.push("Long-term rent valuation available");
    }
    if (strVal?.nightlyRateEstimate || listings.length > 0) {
      recommendedNightlyCents = strVal?.nightlyRateEstimate ?? listings[0]?.nightPriceCents ?? undefined;
      if (recommendedNightlyCents) factors.push("STR rate from valuation or listing");
    }
