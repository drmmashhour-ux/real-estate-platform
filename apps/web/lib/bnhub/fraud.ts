...
export async function getOrCreateFraudAlert(params: {
  ...
  const existingCandidates = await prisma.fraudAlert.findMany({
    where: {
  ...
  const existing = existingCandidates.find((alert) => {
    if (!Array.isArray(alert.signalIds)) return false;
    const signalIds = alert.signalIds as string[];
    return params.signalIds.some((signalId) => signalIds.includes(signalId));
  });
