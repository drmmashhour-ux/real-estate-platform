...
export async function saveValuation(
  ...
  if (result.valuationType === "sale") {
    ...
    data.annualRevenueEstimate = null;
  }
  ...
  const row = await prisma.propertyValuation.create({
    data: {
      ...
      explanation: data.explanation as Prisma.InputJsonValue,
      comparablesSummary: data.comparablesSummary == null ? Prisma.JsonNull : (data.comparablesSummary as Prisma.InputJsonValue),
      seasonalitySummary: data.seasonalitySummary == null ? Prisma.JsonNull : (data.seasonalitySummary as Prisma.InputJsonValue),
    },
  });
  return row.id;
}
