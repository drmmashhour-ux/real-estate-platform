...
export async function upsertIdentityRiskProfile(input: IdentityRiskInput) {
  const riskReasons =
    input.riskReasons == null
      ? undefined
      : Array.isArray(input.riskReasons)
        ? input.riskReasons.filter((reason): reason is string => typeof reason === "string")
        : (input.riskReasons as Record<string, unknown>);
  const riskReasonsJson = riskReasons as Prisma.InputJsonValue | undefined;
...
export async function recordIdentityEvent(params: {
  ...
  const eventData = params.eventData as Prisma.InputJsonValue | undefined;
