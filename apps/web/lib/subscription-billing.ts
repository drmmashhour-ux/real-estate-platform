...
import type { PlanSubscriptionStatus, BillingCycle, SubscriptionPlanModule } from "@prisma/client";
import type { Prisma } from "@prisma/client";
import { recordRevenueEntry } from "@/lib/revenue-intelligence";

export type SubscriptionStatus = PlanSubscriptionStatus;
export type { BillingCycle, SubscriptionPlanModule };
...
export async function createSubscription(params: {
  ...
  const trialEnd = params.trialDays
    ...
    : null;
  const status: PlanSubscriptionStatus =
...
export async function recordBillingEvent(params: {
  ...
  return prisma.planBillingEvent.create({
    data: {
...
/** Get billing events for a user (usage charges, e.g. Design Studio after trial). Shown on the bill. */
export async function getBillingEventsForUser(userId: string, limit = 50) {
  return prisma.planBillingEvent.findMany({
    where: { userId },
