/**
 * Phase 3 — Deal pipeline (anonymous smoke).
 * Run: pnpm exec tsx scripts/phase3-validation.ts
 */
import {
  assertServerReady,
  fetchJson,
...
async function main(): Promise<void> {
  printPhaseHeader(3, "Deal pipeline + committee");
  ...
  await run("GET /api/deals/pipeline/summary (→ 401 anonymous)", async () => {
    const { res } = await fetchJson(`${base}/api/deals/pipeline/summary`);
