/**
 * Phase 4 — Capital stack + lender workflow (anonymous smoke).
 * Run: pnpm exec tsx scripts/phase4-validation.ts
 */
import {
  assertServerReady,
  fetchJson,
...
async function main(): Promise<void> {
  printPhaseHeader(4, "Capital stack + lender workflow");
  ...
  await run("GET /api/capital/pipeline/summary (→ 401 anonymous)", async () => {
    const { res } = await fetchJson(`${base}/api/capital/pipeline/summary`);
