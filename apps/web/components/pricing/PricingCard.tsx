  ...
  return (
    <div
      className={`flex h-full flex-col rounded-2xl border p-6 sm:p-8 ${
        highlighted
          ? "border-[#C9A646]/55 bg-gradient-to-b from-[#C9A646]/12 via-black/40 to-black/60 shadow-xl shadow-[#C9A646]/10 ring-1 ring-[#C9A646]/25"
          : "border-white/10 bg-white/[0.02]"
      }`}
    >
      <div className="min-h-[3rem]">
        {badge ? (
          <p className="text-xs font-semibold uppercase tracking-wider text-[#C9A646]">{badgeText[badge]}</p>
        ) : (
      ...
      <div className="mt-5">
        <p className="text-3xl font-bold tracking-tight text-[#C9A646]">{priceLabel}</p>
      ...
      <Link
        ...
        className={`mt-8 inline-flex w-full items-center justify-center rounded-xl px-5 py-3.5 text-center text-sm font-semibold transition ${
          highlighted
            ? "bg-gradient-to-r from-[#C9A646] via-[#d4b45c] to-[#a88a2e] text-black shadow-lg shadow-[#C9A646]/25 hover:brightness-110"
            : "border border-white/20 text-white hover:border-[#C9A646]/50 hover:bg-white/5"
        }`}
      >
        {ctaLabel}
      </Link>
