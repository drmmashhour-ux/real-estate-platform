...
export function PropertyDealInsightRail({ listingId, trustScore, analysis, missingItemsCount }: Props) {
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-premium-gold/25 bg-[#0D0D0D] p-5 shadow-[0_16px_48px_rgba(0,0,0,0.55)]">
      <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-premium-gold">Decision summary</p>
      ...
      <Link
        ...
        className="flex w-full items-center justify-center rounded-full bg-premium-gold py-3 text-sm font-bold text-black shadow-[0_8px_24px_rgb(var(--premium-gold-channels) / 0.25)] transition hover:bg-[#d4b35c]"
      >
        Fix issues now
      </Link>
    </div>
