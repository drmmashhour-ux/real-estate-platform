          ...
          {steps.map((line, i) => (
            <AnimatedReveal key={line} delayMs={i * 45}>
              <div className="flex h-full gap-4 rounded-2xl border border-white/10 bg-white/[0.02] p-6">
                <span
                  className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-premium-gold/40 bg-premium-gold/10 text-xs font-bold text-premium-gold"
                  aria-hidden
                >
                  {i + 1}
                </span>
