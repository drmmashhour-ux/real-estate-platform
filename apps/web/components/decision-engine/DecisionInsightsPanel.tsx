...
export function DecisionInsightsPanel({ listingId, initial }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-premium-gold/20 bg-[#121212] p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-premium-gold">Decision engine</p>
        ...
        <button
          ...
          disabled={loading}
          className="shrink-0 rounded-full border border-premium-gold/40 bg-premium-gold/10 px-4 py-2 text-xs font-semibold text-premium-gold transition hover:bg-premium-gold/20 disabled:opacity-50"
        >
      ...
      {!data ? (
        <div className="mt-6 rounded-xl border border-white/10 bg-[#0D0D0D] p-4">
          ...
          <button
            ...
            onClick={() => void runFullEngine()}
            disabled={loading}
            className="mt-3 rounded-full bg-premium-gold px-5 py-2.5 text-sm font-bold text-black disabled:opacity-50"
          >
            Run decision engine
          </button>
        </div>
