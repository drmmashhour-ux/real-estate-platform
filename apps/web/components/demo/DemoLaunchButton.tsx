...
export function DemoLaunchButton() {
  ...
  return (
    <div className="pointer-events-auto fixed bottom-5 left-5 z-[10030] flex max-w-[min(100vw-2.5rem,280px)] flex-col gap-2 rounded-2xl border border-white/10 bg-[#121212]/95 p-3 text-xs text-slate-300 shadow-xl backdrop-blur">
      <p className="font-semibold text-[#C9A646]">Guided tour</p>
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          disabled={isActive}
          onClick={() => startDemo("standard_user_tour", "launch_button")}
          className="rounded-lg bg-[#C9A646]/20 px-2 py-1.5 font-medium text-[#C9A646] hover:bg-[#C9A646]/30 disabled:opacity-40"
        >
          Standard
        </button>
        <button
      ...
      <Link href="/demo" className="text-[11px] text-slate-500 underline hover:text-slate-300">
        Demo overview
      </Link>
