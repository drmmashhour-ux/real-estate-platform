...
export async function createOrLink(input: CreateOrLinkInput): Promise<CreateOrLinkResult> {
  ...
  if (existing) {
    const duplicateOutcome = await checkDuplicateOutcome(existing.propertyIdentityId, input.listingId, input.listingType, input.linkedByUserId);
    const { linkId, linkStatus } = await linkToExisting(existing.propertyIdentityId, existing.propertyUid, input, duplicateOutcome);
    return {
      propertyIdentityId: existing.propertyIdentityId,
  ...
  const { propertyIdentityId, propertyUid, linkId } = await createIdentityAndLink(input, linkStatus);
  return {
    propertyIdentityId,
    propertyUid,
    linkId,
