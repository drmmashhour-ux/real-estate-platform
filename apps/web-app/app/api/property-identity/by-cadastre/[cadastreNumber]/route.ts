...
/**
 * GET /api/property-identity/by-cadastre/:cadastreNumber
 * Returns property identity (if any) for the given cadastre number.
 */
export async function GET(
  request: NextRequest,
  context: { params: Promise<{ cadastreNumber: string }> }
) {
  try {
    ...
    const cadastreNumber = decodeURIComponent((await context.params).cadastreNumber);
    if (!cadastreNumber.trim()) {
      return Response.json({ error: "cadastreNumber required" }, { status: 400 });
    }
