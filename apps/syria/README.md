# Hadiah Link — Syria app (`apps/syria`)

Consumer marketplace for Syria under the **Hadiah Link / هدية لينك** product identity: buy, long-term rent, and BNHub-style stays where enabled — manual-first payments and admin-controlled payouts (no Stripe).

## Product identity

- **Hadiah Link** is the Syria-facing product name for this app (`apps/syria`). It uses its **own theme, UX, and positioning** — isolated from LECIPM black/gold branding.
- Canonical brand rules live in **`docs/syria-platform/darlink-brand.md`** (internal filename; product is Hadiah Link).
