...
export default async function SybnbListingPage(props: Props) {
  ...
  return (
    <div className="space-y-8 pb-28 lg:grid lg:grid-cols-12 lg:gap-8 lg:pb-10">
      <div className="space-y-6 lg:col-span-8">
        <PropertyImageGallery images={listing.images} title={pickListingTitle(listing, locale)} />
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-2xl font-semibold tracking-tight text-neutral-900 [dir=rtl]:text-right">{pickListingTitle(listing, locale)}</h1>
            {listing.verified || listing.listingVerified ? <VerifiedBadge label={tListing("verifiedBadge")} /> : null}
          </div>
        ...
        {fromAmenities.length > 0 ? (
          <div>
            <h2 className="text-sm font-semibold text-neutral-800">{t("amenities")}</h2>
        ...
        <div className="lg:sticky lg:top-24">
          <div className="rounded-2xl border border-neutral-200/90 bg-white p-5 shadow-lg">
            ...
            {canBook ? (
              ...
              <p className="mt-2 text-sm text-neutral-600">{t("ownerNote")}</p>
            ) : !user ? (
