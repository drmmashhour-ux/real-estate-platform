...
async function handleDeclinePOST(req: Request, context: RouteParams): Promise<Response> {
  ...
  if (clientRequestId) {
    ...
    if (hit && hit.kind === "decline" && hit.bookingId === id && hit.userId === user.id) {
  ...
  const r = await hostDeclineSybnbV1Request({
    userId: user.id,
    userRole: user.role,
    bookingId: id,
    clientVersion,
...
export async function POST(req: Request, context: RouteParams): Promise<Response> {
  return sybnbApiCatch(() => handleDeclinePOST(req, context));
}
