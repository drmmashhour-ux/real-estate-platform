...
import { INVESTOR_DEMO_TITLE_PREFIX } from "@/lib/sybnb/investor-demo";
import { assertDemoWriteAllowed } from "@/lib/sybnb/investor-demo-write-guard";
import { INVESTOR_DEMO_META, resetInvestorDemoData } from "@/lib/sybnb/investor-demo-reset";
...
export type InvestorDemoSeedSummary = {
  adminEmail: string;
  bookingSampleIdPrefix: string;
};

/**
 * Seeds investor-demo rows only (DEMO_* emails/titles). Caller must enforce DEMO_DATA_ENABLED / guards.
 */
export async function executeInvestorDemoSeed(prisma: PrismaClient): Promise<InvestorDemoSeedSummary> {
  ...
  console.warn("[investor-demo] seeded demo rows", {
    bookingPreview: b1.id.slice(0, 8),
    adminEmail: admin.email,
  });

  return {
    adminEmail: admin.email,
    bookingSampleIdPrefix: b1.id.slice(0, 8),
  };
}
