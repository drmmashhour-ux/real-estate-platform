...
export async function POST(req: Request) {
  ...
  const scan = await scanBufferBeforeStorage({
    bytes: buf,
    mimeType: mime,
    context: "mortgage_broker_profile_photo",
  });
  if (!scan.ok) {
  ...
  const relative = `/uploads/mortgage-brokers/${session.broker.id}.${ext}`;
  const dir = path.join(process.cwd(), "public", "uploads", "mortgage-brokers");
  ...
  const updated = await prisma.mortgageBroker.update({
    ...
    data: { profilePhotoUrl: relative },
  });

  return NextResponse.json({ ok: true, profilePhotoUrl: updated.profilePhotoUrl });
}
