import type {
  ...
  ListingAnalysisOutput,
  ListingAnalysisRecommendation,
} from "../models/index.js";

/**
 * Analyze listing quality and return recommendations.
 * Uses rule-based logic; can be replaced by LLM using prompts/listing-analysis.
 */
export function analyzeListing(input: ListingAnalysisInput): ListingAnalysisOutput {
  ...
  if (!input.title || input.title.length < 10) {
    recommendations.push({
      ...
      priority: "high",
      title: "Improve title",
      suggestion: "Use a clear, descriptive title of 10+ characters (e.g. include location and key feature).",
    });
    score -= 15;
  ...
  if (descLen < 100) {
    recommendations.push({
      ...
      suggestion: "Write at least 100 characters describing the space, neighborhood, and what guests will love.",
    });
    score -= 15;
  } else if (descLen < 300) {
    recommendations.push({
  ...
  if (amenityCount < 3) {
    recommendations.push({
      ...
      suggestion: "Add at least 3–5 amenities (e.g. WiFi, Kitchen, Parking, Air conditioning) to help guests filter.",
    });
    score -= 10;
  }
  ...
  if (photoCount < 5) {
    recommendations.push({
      ...
      suggestion: "Listings with 5+ high-quality photos convert better. Add clear shots of each room and the exterior.",
    });
    score -= 10;
  }
