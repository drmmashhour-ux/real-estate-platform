...
const logger = new StructuredLogger({ service: "transaction-service", environment });
...
export const server = createServer((req, res) => {
  ...
  if (req.method === "GET" && req.url === "/health") {
    return json(res, 200, createHealth("transaction-service", environment));
  }
...
if (process.env["NODE_ENV"] !== "test") {
  server.listen(Number(process.env["PORT"] ?? 4101), () => {
    logger.info(createCorrelationId(), "service.started", { port: process.env["PORT"] ?? 4101 });
  });
}
